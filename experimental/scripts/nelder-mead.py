...
from deepinv.optim.phase_retrieval import cosine_similarity, generate_signal, spectral_methods
...
def main():
    ...
    solution = sp.optimize.minimize(fun=loss_func, x0=hist, method='Nelder-Mead', options={'maxiter': 1000}, callback=callback)

    loss_history = pd.DataFrame(loss_history, columns=['loss'])
    np.save('x_history.npy', x_history)
    pd.save_csv(loss_history, 'loss_history.csv')
